...
> 设计细节：为了不引入全新的概念，采用独立的函数来创建和监听响应式的状态等
...
    }, {
	    flush: "pre" // 默认值为pre,这个执行顺序是先执行watchEffect再执行render
	    // post: 先执行render后执行watchEffect
    });
    // 执行后，停止监听watchEffect的变化
    const stopWatch = () => {
      stop && stop();
    };
    return {
...
export default {
	setup() {
		const users = reactive([{ name: "小红" }, { name: "小李" }]);	
	    const lis = ref([]);
	    onMounted(() => {	
...
#### 如何使用ref获取子组件的值与调用子组件的方法
可以使用 defineExpose 将子组件的参数导出
...
export default {
	...
	    // const { x: XX, y: YY } = position; // 直接进行解构reactive数值时，会使响应式数据丢失，应该使用toRefs
...
composition API出现的动机以及目的：
- 更好的代码组织与逻辑复用
	- 代码组织对比vue2，vue3基于功能而不是基于选项；options api的反例，组织功能更加统一便于维护
	- 逻辑复用：本质就是函数，天然支持复用；mixin的弊端(命名冲突，来源不清晰)
- 更好的类型推导（ts -> function）
